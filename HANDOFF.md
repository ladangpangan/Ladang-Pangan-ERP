...
- **Fitur #5 — Modul CRM (Pipeline Prospek/Leads + Follow-up)**: SELESAI, baru saja di-push, **redeploy Railway sedang/baru selesai berjalan — belum diverifikasi visual oleh user**. Bukan dari 4 fitur asli, diminta user di tengah sesi ini. Tabel baru `leads` + `lead_tasks` (Mongo-authoritative, "Phase 11", `lib/db/crm-mongo.js` — pola diff-persist sama seperti `potx-mongo.js`). Endpoint di `route.js`: `GET/POST /leads`, `GET/PATCH /leads/:id`, `POST /leads/:id/stage`, `POST /leads/:id/convert`, `POST/PATCH/DELETE /leads/:id/tasks[/:taskId]`, `GET /lead-tasks` (panel "Tugas Saya" lintas-lead, filter `overdue=1`). Archive/restore numpang mekanisme generik `ARCHIVABLE` yang sudah ada (tinggal daftar `'leads'`), bukan endpoint baru.
  Keputusan produk (dari klarifikasi user): pipeline 5 tahap (Kontak Awal → Penawaran → Negosiasi → Deal | Gagal, Deal/Gagal final/tidak bisa diubah lagi), leads **terpisah** dari Contacts (baru jadi Contact saat di-"Tandai Deal", ditautkan ke Contact yang **sudah ada** — belum ada form buat-Contact-baru inline, sengaja diarahkan ke halaman Contacts biar tidak duplikasi logika generate kode kontak), semua role bisa akses menu-nya tapi role non-manajemen (bukan admin/supervisor/direktur) **selalu** dibatasi cuma lihat/kelola leads & tugas miliknya sendiri (dipaksa di server, bukan cuma disembunyikan di UI).
  Frontend: `app/dashboard/leads/page.js` (panel Tugas Saya + papan pipeline 5 kolom) dan `app/dashboard/leads/[id]/page.js` (edit, tombol aksi tahap, follow-up tasks, dialog Tandai Deal). Menu baru "Pipeline Prospek" di sidebar (section CRM).
  **Testing**: backend diuji menyeluruh lewat HTTP asli ke dev server (scoping per-role, blokir transisi ke Deal langsung, wajib lostReason utk Gagal, blokir setelah status final, archive khusus role manajemen) — semua lolos. Frontend HANYA bisa dites sampai "redirect ke /login dengan benar, tidak crash" karena sandbox ini tidak ada akses Mongo (dibutuhkan Better Auth utk sesi login) — **belum ada verifikasi visual asli, harus dicoba user di Railway staging**.
- **Railway staging** — source branch service `erp-staging` sempat ketinggalan (masih nunjuk ke branch lama `claude/audit-hostinger-vps-deploy-vpmqyi`, jadi kerjaan sesi ini belum ke-deploy). Sudah dialihkan ke `claude/serene-brahmagupta-wxd66w` (lewat Railway MCP tools) — sekarang **tidak auto-deploy dari webhook GitHub secara andal** (harus dipicu manual lewat `connect-service-source` Railway MCP tool ulang tiap ada push baru — sudah dilakukan berkali-kali sesi ini, cek `mcp__Railway__list-deployments` kalau ragu kode terbaru sudah live atau belum). **Perlu diingat**: kalau ganti strategi branching nanti (mis. setelah PR ke `main`), source Railway ini perlu dialihkan lagi.
...
5. **Modul CRM (Pipeline Prospek/Leads)** — diminta di tengah sesi ini (bukan salah satu dari 4 fitur awal). **SELESAI dikerjakan, redeploy Railway baru saja jalan — belum diverifikasi visual oleh user**, lihat detail lengkap di atas.
...
2. **User coba Fitur #5 (CRM/Pipeline Prospek) di Railway staging** — menu "Pipeline Prospek" di sidebar (section CRM). Cek: bikin lead, pindah tahap, tambah follow-up task, tandai selesai, dan alur "Tandai Deal" (perlu Contact Customer yang sudah ada dulu). Ini yang PALING baru & PALING belum tervalidasi visual.
3. User set `AGENT_API_KEY` di `.env` server (VPS) / Railway variables & coba integrasi Hermes (Fitur #4) — pilih jalur REST langsung atau MCP server sesuai kemampuan Hermes, lihat `deploy/HERMES_INTEGRATION.md`. Kalau mau Hermes bisa memicu rollback/hapus juga, set `AGENT_ROLE=direktur` (default `admin` TIDAK cukup untuk endpoint itu lagi).
4. User jalankan `check_stale_deletes.js` di VPS kapan sempat, laporkan hasilnya (tidak lagi blocking, tapi masih relevan untuk bersih-bersih data lama).
5. Selesaikan pembersihan riwayat `main` (`data/erp.db`) dari akses yang bisa push ke `main` (lihat "Temuan lain") — sebelum bikin PR dari branch ini.
6. Masuk ke daftar keamanan pra-produksi (ganti password, rotate Mongo, Nginx+HTTPS, PR).
