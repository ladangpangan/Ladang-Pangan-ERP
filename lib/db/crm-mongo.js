// =====================================================================
// CRM (leads + follow-up tasks) — MongoDB-authoritative store (Migration Phase 11).
// -------------------------------------------------------------------
// Same diff-persist philosophy as potx-mongo.js / inventory-mongo.js: multi-replica production
// needs a shared source of truth, so per-pod SQLite is a mirror hydrated before reads and
// diff-persisted back to Mongo after writes.
//   captureSnapshot(request, sqlite)     (after hydrate, before the mutation)
//   persistSnapshotDiff(request, sqlite) (push ONLY added/changed/removed rows back to Mongo)
//
// Tables: leads (parent), lead_tasks (child, lead_id, ON DELETE CASCADE).
// =====================================================================
import { getMongoDb } from '@/lib/db/mongo';

const TABLES = ['leads', 'lead_tasks'];
const META = 'mongo_migration';
const SEED_KEY = 'crm_v1';
...
async function ensureIndexes() {
  if (_indexed) return;
  try {
    await col('leads').createIndex({ id: 1 }, { unique: true });
    await col('lead_tasks').createIndex({ id: 1 }, { unique: true });
    await col('lead_tasks').createIndex({ lead_id: 1 });
    _indexed = true;
  } catch { /* best-effort */ }
}
...
async function seed(sqlite) {
  ...
  } catch (e) { console.error('[crm-mongo] seed failed:', e?.message || e); }
...
export async function ensureReady(sqlite) {
  ...
  catch (e) { console.error('[crm-mongo] ensureReady failed:', e?.message || e); return false; }
}
...
export function captureSnapshot(request, sqlite) {
  try {
    ...
    _snap.set(request, m);
  } catch (e) { console.error('[crm-mongo] captureSnapshot failed:', e?.message || e); }
}
...
export async function persistSnapshotDiff(request, sqlite) {
  ...
  try {
    ...
    return total;
  } catch (e) { console.error('[crm-mongo] persistSnapshotDiff failed:', e?.message || e); return false; }
}
