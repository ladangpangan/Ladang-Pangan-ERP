...
async function ensureIndexes() {
  if (_indexed) return;
  try { for (const { name, pk } of TABLES) await col(name).createIndex({ [pk]: 1 }, { unique: true }); _indexed = true; }
  catch { /* best-effort */ }
}
...
export async function hydrateToSqlite(sqlite) {
  const data = {};
  for (const { name } of TABLES) data[name] = await col(name).find({}, { projection: { _id: 0 } }).toArray();
