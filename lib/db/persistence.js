...
// Restore the SQLite file from Mongo. MongoDB (Atlas) is the AUTHORITATIVE durable store:
// on EVERY boot, if a valid backup exists we restore it — even when this container already
// shipped a baseline SQLite file. This guarantees a redeploy always loads the latest live
// data and NEVER lets a stale bundled baseline overwrite production data. If Mongo is
// unreachable or there is no backup yet, we keep the local file (the seed-snapshot fallback
// in seed.js handles a truly fresh install).
