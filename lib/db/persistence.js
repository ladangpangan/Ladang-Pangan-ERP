...
// Prefer ATLAS_MONGO_URL (user's own Atlas) over the platform-injected MONGO_URL. See mongo.js.
const MONGO_URL = process.env.ATLAS_MONGO_URL || process.env.MONGO_URL || '';
...
// Restore the SQLite file from Mongo on a genuinely FRESH disk only (file missing/empty).
// This was originally unconditional (restore on every boot) to protect Emergent's ephemeral
// pods, where the local disk never survives a redeploy so it is always empty at this point
// anyway. On a host with a persistent volume (Docker on a VPS, etc.) the local file DOES
// survive restarts, so restoring unconditionally would silently roll back every write made
// since the last debounced/periodic backup on every `docker compose restart` or crash-restart.
// Set FORCE_MONGO_RESTORE=1 to explicitly force a restore over an existing local file (disaster
// recovery only — this discards any local-only writes not yet backed up).
export async function restoreDbFromMongoIfNeeded() {
  if (!MONGO_URL) return { restored: false, reason: 'no-mongo-url' };
  const localExists = fs.existsSync(DB_PATH) && fs.statSync(DB_PATH).size > 0;
  if (localExists && process.env.FORCE_MONGO_RESTORE !== '1') {
