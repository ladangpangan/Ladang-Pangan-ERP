...
export const salesOrder = sqliteTable('sales_order', {
  ...
  shippingAccountCode: text('shipping_account_code'), // kode COA Kas/Bank spesifik yang dipakai bayar ongkir (opsional)
  ...
  // Bukti & status pengembalian cashback (operasional, bukan jurnal — jurnal cashback otomatis saat invoice)
  cashbackRefunded: integer('cashback_refunded', { mode: 'boolean' }).notNull().default(false),
  cashbackRefundedAt: text('cashback_refunded_at'), // tanggal transfer cashback (ISO)
  cashbackRefundNote: text('cashback_refund_note'),
  cashbackRefundedBy: text('cashback_refunded_by'), // email user yang menandai
  cashbackProofKey: text('cashback_proof_key'), // storage key file bukti di disk
  cashbackProofName: text('cashback_proof_name'),
  cashbackProofType: text('cashback_proof_type'),
  // Jumlah cashback yang DIPOTONG untuk menutup kekurangan bayar invoice SO ini (netting), alih-alih
  // menagih kekurangan itu terpisah. cashback aktual yang ditransfer = cashbackAmount - cashbackDeduction.
  cashbackDeduction: real('cashback_deduction').notNull().default(0),
...
export const salesPayments = sqliteTable('sales_payments', {
  ...
  accountCode: text('account_code'), // COA Kas/Bank penerimaan (1-11xx)
...
export const salesReturns = sqliteTable('sales_returns', {
  ...
  // Dua titik ukur opsional untuk memisahkan susut retur (di tangan customer, sebelum dikirim
  // balik) dari susut pengembalian retur (transit balik ke gudang). 0 = belum diukur.
  weightAtPickup: real('weight_at_pickup').notNull().default(0), // berat saat retur dikirim/dilaporkan customer
  weightAtWarehouse: real('weight_at_warehouse').notNull().default(0), // berat saat retur tiba di gudang
...
export const purchaseOrder = sqliteTable('purchase_order', {
  ...
  shippingAddress: text('shipping_address'), // tujuan kirim dropship (supplier kirim langsung ke sini, bukan ke gudang kita)
  salesOrderId: text('sales_order_id'), // link balik ke SO Dropship yang meng-generate PO ini
  additionalCost: real('additional_cost').notNull().default(0), // total ongkir/biaya tambahan
  additionalCostBearer: text('additional_cost_bearer').notNull().default('company'), // company (kita) | supplier (pemasok)
  additionalCostPayMethod: text('additional_cost_pay_method').notNull().default('utang'), // tunai | transfer | utang (ke pemasok)
  totalAmount: real('total_amount').notNull().default(0),
  dpAmount: real('dp_amount').notNull().default(0),
  paidAmount: real('paid_amount').notNull().default(0),
  paymentStatus: text('payment_status').notNull().default('unpaid'), // unpaid | partial | paid
...
export const purchasePayments = sqliteTable('purchase_payments', {
  ...
  amount: real('amount').notNull().default(0),
  method: text('method').notNull().default('Transfer'), // Transfer | Tunai | QRIS
  accountCode: text('account_code'), // COA Kas/Bank sumber pembayaran (1-11xx)
  reference: text('reference'),
  isDp: integer('is_dp', { mode: 'boolean' }).notNull().default(false),
  notes: text('notes'),
  createdBy: text('created_by'),
...
export const commissionPayments = sqliteTable('commission_payments', {
  ...
  amount: real('amount').notNull().default(0),
  method: text('method').notNull().default('Transfer'), // Transfer | Tunai | QRIS
  accountCode: text('account_code'), // kode akun kas/bank sumber pembayaran komisi (untuk jurnal)
  reference: text('reference'),
  notes: text('notes'),
  createdBy: text('created_by'),
  createdAt: integer('created_at', { mode: 'timestamp' }).notNull().default(sql`(unixepoch())`),
...
export const leads = sqliteTable('leads', {
  ...
  address: text('address'),
  city: text('city'),
  source: text('source').notNull().default('lainnya'), // referral | cold_call | pameran | website | media_sosial | lainnya
  stage: text('stage').notNull().default('Kontak Awal'), // Kontak Awal | Penawaran | Negosiasi | Deal | Gagal
  estimatedValue: real('estimated_value').notNull().default(0),
  lostReason: text('lost_reason'),
  // Email pemilik lead (PIC), bukan FK ke tabel user SQLite — user asli hidup di MongoDB (Better
  // Auth), tabel user SQLite lokal cuma sisa skema lama & belum tentu ke-hydrate. Pola ini sama
  // dengan createdBy/supervisorActedBy dkk di seluruh app (plain email, bukan FK).
...
export const leadTasks = sqliteTable('lead_tasks', {
  ...
  assignedTo: text('assigned_to').notNull(), // email, sama pola dengan picEmail di atas
  notes: text('notes'),
