...
// Marketplace Orders — inbox for orders received from the ladangpangan.id
// landing page (a separate app). Each row is one order; the Contact is
// matched/created as soon as it arrives, but the Sales Order is only
// created when staff clicks "Proses SO" on the Market Place Order page
// (app/dashboard/marketplace-orders), since item-to-product matching may
// need a human when a landing-page product name doesn't match any ERP
// product.
export const marketplaceOrders = sqliteTable('marketplace_orders', {
  id: text('id').primaryKey(),
  orderId: text('order_id').notNull().unique(), // landing page's own order id, e.g. LPI-...
  customerName: text('customer_name').notNull(),
  customerPhone: text('customer_phone').notNull(),
  customerAddress: text('customer_address'),
  contactId: text('contact_id').references(() => contacts.id),
  items: text('items').notNull(), // JSON array [{ name, price, qty }]
