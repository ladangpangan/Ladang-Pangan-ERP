...
// Prefer ATLAS_MONGO_URL when provided. The Emergent platform injects its own MONGO_URL at
// deploy time (its managed cluster), which would otherwise override the .env value. Using a
// dedicated var that the platform does NOT touch lets production connect to the user's own
// Atlas cluster (where all data + auth users live). Falls back to MONGO_URL then localhost.
...
export function getMongoClient() {
  if (!_client) {
    // The driver's default server-selection timeout is ~30s. Several boot-time calls
    // (seedAuthUsers, refreshUserCache in lib/db/boot.js) AWAIT this client directly, so an
    // unreachable Mongo (e.g. no ATLAS_MONGO_URL/MONGO_URL configured, as in a bare preview
    // deploy) stalls Next.js request handling for the full 30s on every cold start — long
    // enough to fail platform health checks outright. A real Atlas connection establishes in
    // well under a second, so a short timeout here only affects the already-broken case.
