...
export function syncLedger(sqlite, { createdBy = null } = {}) {
  ...
  const tx = sqlite.transaction(() => {
    ...
    try {
      const sos = sqlite.prepare(`SELECT * FROM sales_order WHERE (archived_at IS NULL)
      ...
      for (const so of sos) {
        try {
          ...
          const { dpp, tax } = splitTax(total);
          const lines = [{ code: acc('piutang_usaha'), debit: total, credit: 0, description: `Piutang ${so.so_number}` }];
          ...
          // Cashback / faktur di-up (opsional): selisih di-up dibebankan sbg Beban Komisi/Cashback,
          // dan mengurangi Piutang sehingga sisa tagihan = nilai asli yang benar-benar dibayar customer.
          const cashback = round2(so.cashback_amount || 0);
          if (so.markup_enabled && cashback > 0 && cashback < total && has(acc('beban_komisi'))) {
            lines.push({ code: acc('beban_komisi'), debit: cashback, credit: 0, description: `Cashback/komisi ${so.so_number}` });
            lines.push({ code: acc('piutang_usaha'), debit: 0, credit: cashback, description: `Potongan piutang - cashback ${so.so_number}` });
          }
          // COGS
          let cogs = 0;
    ...
    try {
      ...
      for (const p of rows) {
        try {
          ...
          const lines = [
            { code: cashAcc(p.method), debit: amt, credit: 0, description: `Penerimaan ${p.so_number}` },
    ...
    try {
      const pos = sqlite.prepare(`SELECT po.* FROM purchase_order po WHERE (po.archived_at IS NULL)
      ...
      for (const po of pos) {
        try {
          ...
          lines.push({ code: acc('utang_usaha'), debit: 0, credit: total, description: `Utang ${po.po_number}` });
      ...
      for (const p of rows) {
        try {
          ...
          const lines = [
            ...
            { code: cashAcc(p.method), debit: 0, credit: amt },
          ];
...
export function overview(sqlite) {
  ...
  const kas = balOf(map.kas, 'debit');
  const bank = balOf(map.bank, 'debit');
...
export function salesProfitReport(sqlite, { from = null, to = null } = {}) {
  ...
  const orders = [];
  let tRev = 0, tCogs = 0, cnt = 0;
  for (const so of sos) {
    const date = so.invoice_date || so.order_date;
    if (from != null && date < from) continue;
    if (to != null && date > to) continue;
    const cashback = (so.markup_enabled && Number(so.cashback_amount) > 0) ? round2(so.cashback_amount) : 0;
    const revenue = round2(so.total_amount - cashback);
    ...
    if (!byCustomer[ck]) byCustomer[ck] = { customerId: ck, customer: so.customer_name || '(Tanpa nama)', orders: 0, revenue: 0, cogs: 0, grossProfit: 0 };
    byCustomer[ck].orders++; byCustomer[ck].revenue += revenue; byCustomer[ck].cogs += cogs; byCustomer[ck].grossProfit += gp;
    const d = new Date(date * 1000);
    const mk = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}`;
    if (!byMonth[mk]) byMonth[mk] = { month: mk, orders: 0, revenue: 0, cogs: 0, grossProfit: 0 };
    byMonth[mk].orders++; byMonth[mk].revenue += revenue; byMonth[mk].cogs += cogs; byMonth[mk].grossProfit += gp;
    orders.push({ id: so.id, soNumber: so.so_number, date, customer: so.customer_name || '-', type: so.fulfillment_type, revenue, cogs, grossProfit: gp, margin: revenue ? round2(gp / revenue * 100) : 0 });
    tRev += revenue; tCogs += cogs; cnt++;
  }
  const fin = (o) => { o.revenue = round2(o.revenue); o.cogs = round2(o.cogs); o.grossProfit = round2(o.grossProfit); o.margin = o.revenue ? round2(o.grossProfit / o.revenue * 100) : 0; return o; };
  ...
  return { byCustomer: customers, byMonth: months, orders, totals: { orders: cnt, revenue: round2(tRev), cogs: round2(tCogs), grossProfit: tGp, margin: tRev ? round2(tGp / tRev * 100) : 0 } };
}
