...
export function syncLedger(sqlite, { createdBy = null } = {}) {
  ...
  const tx = sqlite.transaction(() => {
      ...
      for (const so of sos) {
        try {
          ...
          if (shipCost > 0 && has(acc('beban_ongkir'))) {
            // Utamakan akun Kas/Bank spesifik yang dipilih user (shipping_account_code);
            // fallback ke Kas/Bank default berdasarkan metode bayar (legacy) bila belum dipilih.
            let shipCash = (so.shipping_account_code && has(so.shipping_account_code)) ? so.shipping_account_code : null;
            if (!shipCash) shipCash = String(so.shipping_pay_method || 'transfer').toLowerCase().includes('tunai') ? acc('kas') : acc('bank');
            if (has(shipCash)) {
    ...
    // PERUBAHAN (permintaan user): jurnal cashback HANYA dibuat SAAT cashback benar-benar
    // DIKEMBALIKAN (cashback_refunded=1), memakai akun kas/bank yang dipilih (cashback_account)
    // dan tanggal pengembalian. Cashback yang belum dikembalikan TIDAK mengurangi kas di pembukuan.
    try {
      const cbSos = sqlite.prepare(`SELECT * FROM sales_order WHERE archived_at IS NULL AND COALESCE(migrated,0)=0 AND markup_enabled=1 AND cashback_amount > 0
        AND COALESCE(cashback_refunded,0)=1
        AND (invoice_number IS NOT NULL OR pipeline_status IN ('Shipped','Invoiced','Selesai'))`).all();
      for (const so of cbSos) {
        try {
          // cashback_deduction: bagian yang DIPOTONG untuk menutup kekurangan bayar invoice (netting),
          // jadi kas yang benar-benar keluar hanya sebesar sisanya — bukan nilai cashback penuh.
          const cashback = round2((so.cashback_amount || 0) - (so.cashback_deduction || 0));
          ...
          const date = so.cashback_refunded_at || so.invoice_date || so.order_date;
          post({
    ...
    // 2c) KOMISI DROPSHIPPER — jurnal saat komisi DIBAYAR (permintaan user) ------
    // Dr Beban Komisi ; Cr Kas/Bank (dari akun yang dipilih saat pembayaran).
    // Dikelompokkan per pembayaran (commission_payments); satu pembayaran bisa melunasi
    // beberapa record komisi sekaligus. Komisi yang belum dibayar tidak dijurnal.
    try {
      const cpays = sqlite.prepare(`SELECT cp.*, c.display_name AS ds_name, c.code AS ds_code
        FROM commission_payments cp LEFT JOIN contacts c ON c.id = cp.dropshipper_id`).all();
      for (const p of cpays) {
        try {
          const amount = round2(p.amount || 0);
          if (amount <= 0) continue;
          const cashCode = (p.account_code && has(p.account_code)) ? p.account_code : (has(acc('bank')) ? acc('bank') : acc('kas'));
          if (!has(acc('beban_komisi')) || !has(cashCode)) continue;
          const dsName = p.ds_name || p.ds_code || '';
          const date = p.payment_date || Math.floor(Date.now() / 1000);
          post({
            date, description: `Pembayaran komisi dropshipper${dsName ? ' ' + dsName : ''}`,
            sourceType: 'COMMISSION_PAY', sourceId: p.id, sourceNumber: p.reference || null, sourceKey: `COMPAY:${p.id}`,
            lines: [
              { code: acc('beban_komisi'), debit: amount, credit: 0, description: `Komisi${dsName ? ' ' + dsName : ''}` },
              { code: cashCode, debit: 0, credit: amount, description: `Bayar komisi${dsName ? ' ' + dsName : ''}` },
            ],
          });
        } catch (e) { console.error('sync komisi pay', p?.id, e?.message); }
      }
    } catch (e) { console.error('sync komisi section', e?.message); }
...
export function listCashbook(sqlite, { from = null, to = null, type = null, limit = 300 } = {}) {
  ...
  return entries.map((e) => {
    ...
    return {
      ...
      createdBy: e.created_by, created_by: e.created_by,
    };
  });
}
