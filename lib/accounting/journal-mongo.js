...
// Merge MongoDB stock_ledger docs into the per-pod SQLite mirror (upsert by id) so Kartu Stok
// shows movements recorded on ALL replicas. Also RECONCILES deletions: local rows that are no
// longer in Mongo AND older than a grace window are removed, so a Mongo-side wipe/removal
// propagates to every replica on its next hydrate (Kartu Stok used to be merge-only => deletions
// never propagated). The grace window preserves a just-written local row whose fire-and-forget
// Mongo insert may not have landed yet (created_at is stored in SECONDS, matching recordLedger).
export async function hydrateStockLedgerToSqlite(sqlite) {
  try {
    ...
    const delStmt = sqlite.prepare('DELETE FROM stock_ledger WHERE id = ?');
    const graceCutoff = Math.floor(Date.now() / 1000) - 300; // seconds
    const tx = sqlite.transaction(() => {
      for (const r of rows) ins.run(...SL_COLS.map((c) => (r[c] === undefined ? null : r[c])));
      const local = sqlite.prepare('SELECT id, created_at FROM stock_ledger').all();
      for (const l of local) {
        if (!mongoIds.has(l.id) && Number(l.created_at || 0) < graceCutoff) delStmt.run(l.id);
      }
    });
    tx();
    return rows.length;
