...
export function drawDocHeader(doc, opts = {}) {
  ...
  if (centered) {
    ...
    const ct = String(title || '');
    const cCs = 0.7;
    const cMax = pageWidth - margin * 2 - 8;
    const cMeasure = (sz) => { doc.setFontSize(sz); return doc.getTextWidth(ct) + cCs * Math.max(0, ct.length - 1); };
    let cSize = 13.5;
    while (cSize > 9 && cMeasure(cSize) > cMax) cSize -= 0.5;
    doc.setFontSize(cSize);
    doc.setCharSpace(cCs);
    doc.text(ct, pageWidth / 2, divY + 8, { align: 'center' });
    doc.setCharSpace(0);
  ...
  // AUTO-FIT + charSpace-accurate positioning. NOTE: jsPDF's align:'right' does NOT account for
  // setCharSpace, so a spaced title overflowed past the right margin. We therefore measure the REAL
  // drawn width (glyphs + char spacing) and position the title LEFT-aligned at (right margin - width)
  // so its right edge lands exactly on the margin. Also shrink long titles to a 64mm zone.
  const t = String(title || '').toUpperCase();
  const maxTitleW = 64;
  const cs = 1.3;
  doc.setFont('helvetica', 'bold');
  const drawnW = (sz) => { doc.setFontSize(sz); return doc.getTextWidth(t) + cs * Math.max(0, t.length - 1); };
  let titleSize = 23;
  while (titleSize > 11 && drawnW(titleSize) > maxTitleW) titleSize -= 0.5;
  doc.setFontSize(titleSize);
  const actualW = doc.getTextWidth(t) + cs * Math.max(0, t.length - 1);
  const titleX = pageWidth - margin - actualW;
  doc.setTextColor(accent[0], accent[1], accent[2]);
  doc.setCharSpace(cs);
  doc.text(t, titleX, 15.5); // left-aligned so the char-spaced right edge = pageWidth - margin
  doc.setCharSpace(0);
  const uW = Math.min(Math.max(actualW, 24), maxTitleW);
