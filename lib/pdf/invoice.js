...
export function generateInvoicePDF(so, opts = {}) {
  ...
  const upTotal = Number(so.totalAmount || 0);
  const realAmt = Number(so.realAmount || 0);
  // Faktur Asli/Internal: skalakan harga per baris ke nilai asli/net
  const factor = (isAsli && upTotal > 0 && realAmt > 0) ? realAmt / upTotal : 1;
  ...
  const items = (so.items || []).map((it, idx) => {
    ...
    const unit = Number(it.unitPrice || 0) * factor;
    const disc = Number(it.discount || 0) * factor;
    return [
      idx + 1,
  ...
  const subtotal = (so.items || []).reduce((a, it) => a + (Number(it.shippedWeight || 0) > 0 ? Number(it.shippedWeight) : Number(it.weight || 0)) * Number(it.unitPrice || 0), 0) * factor;
  const discount = (so.items || []).reduce((a, it) => a + Number(it.discount || 0), 0) * factor;
  const total = isAsli ? realAmt : Number(so.totalAmount || (subtotal - discount));
  const paid = Number(so.paidAmount || 0);
  const outstanding = isAsli ? (total - paid) : Number(so.outstanding !== undefined ? so.outstanding : total - paid);
