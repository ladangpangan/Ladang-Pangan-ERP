...
export function generateInvoicePDF(so, opts = {}) {
  ...
  // tidak sinkron.
  // Untuk varian 'diup', unitOf() SUDAH memakai markupUnitPrice (harga di-up) per baris — jadi
  // subtotal di bawah ini SUDAH termasuk cashback. Menambah cashbackAmt lagi di sini akan
  // menghitung cashback DUA KALI. Grand Total = subtotal (yang sudah di-up) apa adanya.
  ...
  const total = subtotal - discount + buyerShip;
 ...
 * @param {object} po
 * @param {{ mode?: 'po' | 'invoice' }} [opts] - Dropship PO defaults to the supplier-invoice
 *   layout (mode 'invoice'); pass mode: 'po' to force the initial "PURCHASE ORDER" layout
 *   instead (e.g. the vendor purchase instruction printed before any invoice exists yet).
 */
export function generatePOPDF(po, opts = {}) {
  const st = getSettings(); const accent = accentRGB();
  const doc = new jsPDF({ unit: 'mm', format: 'a4' });
  const pageWidth = doc.internal.pageSize.getWidth();
  const isDropPO = opts.mode ? opts.mode === 'invoice' : !!po.isDropship;
  ...
  let bodyTop = Math.max(partyBottom, metaBottom) + 6;

  // Dropship: tampilkan tujuan kirim (supplier kirim langsung ke customer ini, bukan ke gudang
  // kita) supaya PO/invoice-nya bisa diidentifikasi dikirim kemana.
  if (po.isDropship) {
    const dc = po.dropshipCustomer || {};
    const shipAddr = po.shippingAddress || dc.address || '-';
    const shipLines = [shipAddr, dc.phone ? 'Telp: ' + dc.phone : ''].filter(Boolean);
    bodyTop = partyBlock(doc, MARGIN, bodyTop, 'DIKIRIM KE (DROPSHIP):', dc.displayName, shipLines, pageWidth - MARGIN * 2) + 6;
  }
  ...
  // Catatan / instruksi pembelian ke vendor — supaya jelas tertulis di dokumen yang dikirim,
  // bukan cuma tersimpan di sistem.
  if (po.notes) {
    const noteY = Math.max(finalY, ty + 12) + 3;
    const noteLines = doc.splitTextToSize(po.notes, pageWidth - MARGIN * 2 - 6);
    const noteH = noteLines.length * 4.2 + 10;
    doc.setFillColor(...tint(accent, 0.95)); doc.setDrawColor(...tint(accent, 0.6)); doc.setLineWidth(0.3);
    doc.roundedRect(MARGIN, noteY, pageWidth - MARGIN * 2, noteH, 1.5, 1.5, 'FD');
    doc.setFont('helvetica', 'bold'); doc.setFontSize(8.5); doc.setTextColor(...accent);
    doc.text('CATATAN / INSTRUKSI PEMBELIAN:', MARGIN + 3, noteY + 5.5);
    doc.setFont('helvetica', 'normal'); doc.setFontSize(9); doc.setTextColor(50);
    doc.text(noteLines, MARGIN + 3, noteY + 10.5);
  }
