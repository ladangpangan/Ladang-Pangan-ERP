...
export function generateInvoicePDF(so, opts = {}) {
  ...
  // Berat tagihan mengikuti basis invoice: bila 'received', gunakan berat DITERIMA (fallback ke kirim/awal).
  const billW = (it) => {
    if (so.invoiceWeightBasis === 'received' && Number(it.receivedWeight || 0) > 0) return Number(it.receivedWeight);
    return Number(it.shippedWeight || 0) > 0 ? Number(it.shippedWeight) : Number(it.weight || 0);
  };
  ...
  doc.text(ellipsize(doc, `SO: ${so.soNumber || '-'}   ·   Term: ${so.paymentTerm || '-'}`, tilesW), MARGIN, tilesBottom + 5);
...
export function generateSuratJalanPDF(sj, so) {
  ...
  autoTable(doc, {
    ...
    body: items, theme: 'striped', headStyles: tableHeadStyle(), bodyStyles: { fontSize: 9, cellPadding: 2, overflow: 'linebreak' },
    alternateRowStyles: { fillColor: tint(accent, 0.96) },
    columnStyles: { 0: { halign: 'center', cellWidth: 10 }, 1: { cellWidth: 62, overflow: 'linebreak' }, 2: { halign: 'right', cellWidth: 14 }, 3: { halign: 'center', cellWidth: 28 }, 4: { halign: 'right', cellWidth: 26 }, 5: { halign: 'center' } },
    margin: { left: MARGIN, right: MARGIN },
  });
