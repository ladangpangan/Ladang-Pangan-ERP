...
export function generateSuratJalanPDF(sj, so) {
  ...
  // Items (from SO items) — gunakan berat KIRIM RIIL bila ada, jatuh ke berat SO bila belum diisi
  const items = (so?.items || []).map((it, idx) => {
    const shipW = Number(it.shippedWeight || 0) > 0 ? Number(it.shippedWeight) : Number(it.weight || 0);
    return [
      idx + 1,
      (it.product?.name || '-') +
        (it.product?.sku ? `\n${it.product.sku}` : '') +
  ...
  const totalW = (so?.items || []).reduce((a, it) => a + (Number(it.shippedWeight || 0) > 0 ? Number(it.shippedWeight) : Number(it.weight || 0)), 0);
...
export function generatePOPDF(po) {
  ...
  autoTable(doc, {
    startY: y0 + 40,
    head: [['#', 'Deskripsi', 'Qty', 'Kemasan', 'Berat (kg)', 'Harga/kg', 'Subtotal']],
    body: rows,
    theme: 'striped',
    headStyles: { fillColor: [16, 122, 87], textColor: 255, fontSize: 9, halign: 'center' },
    bodyStyles: { fontSize: 9 },
