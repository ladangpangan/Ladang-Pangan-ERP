...
import {
  ...
  getSettings, getCompany, accentRGB, tint,
...
// Right-aligned key/value row inside the totals column.
...
// A styled meta box (right side) with rows of label/value.
function metaBox(doc, x, y, w, rows, accent) {
  ...
  rows.forEach((line, idx) => {
    ...
    doc.text(line[0], x + 3, ry);
    doc.setFont('helvetica', 'bold'); doc.setTextColor(30);
    doc.text(String(line[1]), x + w - 3, ry, { align: 'right' });
  });
  return y + h;
}

// KEPADA/SUPPLIER party block (left side).
function partyBlock(doc, x, y, label, name, lines) {
  ...
  (lines || []).forEach((ln) => {
    if (!ln) return;
    const arr = doc.splitTextToSize(String(ln), 90);
...
export function generateInvoicePDF(so, opts = {}) {
  ...
  partyBlock(doc, MARGIN, y0, 'KEPADA:', cust.displayName, [addr, cust.phone ? 'Telp: ' + cust.phone : '', cust.taxId ? 'NPWP: ' + cust.taxId : '']);

  const metaX = pageWidth - MARGIN - 70;
  metaBox(doc, metaX, y0, 70, [
  ...
  autoTable(doc, {
    ...
    columnStyles: {
      0: { halign: 'center', cellWidth: 8 }, 1: { cellWidth: 48 }, 2: { halign: 'right', cellWidth: 12 },
  ...
  const boxW = 78; const boxX = pageWidth - MARGIN - boxW;
  ...
  const total = isDiup ? Number(so.totalAmount || 0) + cashbackAmt : Number(so.totalAmount || (subtotal - discount));
  ...
  doc.roundedRect(boxX, ty - 1, boxW, 8.5, 1.5, 1.5, 'F');
  kvRow(doc, labelX, valueX, 'TOTAL', rupiah(total), ty + 4.5, { bold: true, size: 11.5, color: accent }); ty += 12;
  ...
  if (st.showSignature) {
    ...
    doc.text(st.signerName || co.name, pageWidth - MARGIN - 55, sy + 23);
  }
...
export function generateSOPDF(so) {
  ...
  const cust = so.customer || {};
  const addr = [cust.address, cust.city, cust.province].filter(Boolean).join(', ');
  partyBlock(doc, MARGIN, y0, 'KEPADA:', cust.displayName, [addr, cust.phone ? 'Telp: ' + cust.phone : '']);
  ...
  autoTable(doc, {
    startY: y0 + 40,
    head: [['#', 'Deskripsi', 'Qty', 'Kemasan', 'Berat (kg)', 'Harga/kg', 'Diskon', 'Subtotal']],
    body: items, theme: 'striped', headStyles: tableHeadStyle(), bodyStyles: { fontSize: 9, cellPadding: 2 },
    alternateRowStyles: { fillColor: tint(accent, 0.96) },
    columnStyles: { 0: { halign: 'center', cellWidth: 8 }, 1: { cellWidth: 48 }, 2: { halign: 'right', cellWidth: 12 }, 3: { halign: 'center', cellWidth: 22 }, 4: { halign: 'right', cellWidth: 20 }, 5: { halign: 'right', cellWidth: 24 }, 6: { halign: 'right', cellWidth: 22 }, 7: { halign: 'right' } },
    margin: { left: MARGIN, right: MARGIN },
  });
...
export function generateSuratJalanPDF(sj, so) {
  ...
  let py = partyBlock(doc, MARGIN, y0, 'DIKIRIM KEPADA:', sj.shipToName || cust.displayName, [shipAddr, shipPhone ? 'Telp: ' + shipPhone : '']);
  ...
  const items = (so?.items || []).map((it, idx) => {
    ...
    return [
      ...
      it.stock?.coldStorage?.code ? `${it.stock.coldStorage.code}${it.stock.zone?.code ? ' / ' + it.stock.zone.code : ''}` : '-',
    ];
  });
  ...
  autoTable(doc, {
    ...
    body: items, theme: 'striped', headStyles: tableHeadStyle(), bodyStyles: { fontSize: 9, cellPadding: 2 },
    alternateRowStyles: { fillColor: tint(accent, 0.96) },
    columnStyles: { 0: { halign: 'center', cellWidth: 10 }, 1: { cellWidth: 62 }, 2: { halign: 'right', cellWidth: 14 }, 3: { halign: 'center', cellWidth: 28 }, 4: { halign: 'right', cellWidth: 26 }, 5: { halign: 'center' } },
    margin: { left: MARGIN, right: MARGIN },
  });
...
export function generatePOPDF(po) {
  ...
  partyBlock(doc, MARGIN, y0, 'SUPPLIER:', sup.displayName, [sup.phone ? 'Telp: ' + sup.phone : '', sup.address]);
  ...
  autoTable(doc, {
    ...
    body: rows, theme: 'striped', headStyles: tableHeadStyle(), bodyStyles: { fontSize: 9, cellPadding: 2 },
    alternateRowStyles: { fillColor: tint(accent, 0.96) },
    columnStyles: { 0: { halign: 'center', cellWidth: 8 }, 1: { cellWidth: 58 }, 2: { halign: 'right', cellWidth: 12 }, 3: { halign: 'center', cellWidth: 22 }, 4: { halign: 'right', cellWidth: 24 }, 5: { halign: 'right', cellWidth: 28 }, 6: { halign: 'right' } },
    margin: { left: MARGIN, right: MARGIN },
  });
  ...
  let ty = finalY;
  kvRow(doc, labelX, valueX, 'Subtotal', rupiah(subtotal), ty); ty += 5;
  if (addCost > 0) { kvRow(doc, labelX, valueX, 'Biaya Tambahan', rupiah(addCost), ty); ty += 5; }
  ty += 1;
  doc.setFillColor(...tint(accent, 0.85)); doc.roundedRect(boxX, ty - 1, boxW, 8.5, 1.5, 1.5, 'F');
  kvRow(doc, labelX, valueX, 'TOTAL', rupiah(total), ty + 4.5, { bold: true, size: 11.5, color: accent });
...
export function generateTallyInboundPDF(r) {
  ...
  meta.forEach((l, i) => {
    ...
    doc.setFont('helvetica', 'bold'); doc.setTextColor(30); doc.text(String(l[1]), MARGIN + 32, y0 + i * 5.5);
  });
  ...
  autoTable(doc, {
    ...
    body: rows, theme: 'striped', headStyles: tableHeadStyle(), bodyStyles: { fontSize: 9, cellPadding: 2 },
    alternateRowStyles: { fillColor: tint(accent, 0.96) },
    columnStyles: { 0: { halign: 'center', cellWidth: 8 }, 1: { cellWidth: 30, font: 'courier' }, 2: { cellWidth: 60 }, 3: { halign: 'center', cellWidth: 24 }, 4: { halign: 'right', cellWidth: 14 }, 5: { halign: 'right', cellWidth: 24 }, 6: { halign: 'center' } },
    margin: { left: MARGIN, right: MARGIN },
  });
