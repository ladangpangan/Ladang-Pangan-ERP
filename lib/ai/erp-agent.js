  ...
  try {
    switch (action?.type) {
      case 'create_contact': {
        ...
        return { ok: true, message: `Kontak "${a.displayName}" berhasil dibuat (kode ${code}).`, ref: code };
      }
      case 'create_product': {
        const a = action.args || {};
        ...
        return { ok: true, message: `Produk "${a.name}" berhasil dibuat (SKU ${sku}).`, ref: sku };
      }
      ...
      case 'create_purchase_order': {
        ...
        return { ok: true, message: `Draft Purchase Order ${poNumber} berhasil dibuat (${a.items.length} item, total ${money(total)}).`, ref: poNumber };
      }
      case 'create_sales_order': {
        ...
        return { ok: true, message: `Draft Sales Order ${soNumber} berhasil dibuat (${a.items.length} item, total ${money(total)}).`, ref: soNumber };
      }
      default:
...
8. MEMBUAT ORDER (Purchase Order / Sales Order): sebelum memanggil tool create_purchase_order atau create_sales_order, pastikan Anda sudah tahu: (a) supplier/customer, (b) daftar produk beserta jumlah atau berat (kg), dan (c) harga per item. Jika ada yang belum jelas, tanyakan dulu. Harga boleh dikosongkan (akan memakai harga dasar produk). Order selalu dibuat berstatus "Draft" dan tetap butuh konfirmasi user. Setelah tool mengembalikan confirmation_required, jelaskan ringkas isi order lalu minta user menekan tombol Konfirmasi.`;
}
...
export async function runAgent({ message, history = [], user, sessionId }) {
  ...
  try {
    ...
    return { ok: true, answer, pendingActions, toolLog, usage: response.usage || null };
  } catch (e) {
