...
function previewOpenOrderBuilder(db, args) {
  let orderType = (args.orderType || '').toUpperCase();
  if (orderType !== 'SO' && orderType !== 'PO') orderType = null;
  const prefill = {};
  if (args.fulfillmentType) prefill.fulfillmentType = args.fulfillmentType === 'dropship' ? 'dropship' : 'stock';
  if (args.contact) {
    const hint = orderType === 'PO' ? 'Supplier' : 'Customer';
    const c = resolveContact(db, args.contact, hint);
    if (c) { prefill.contactId = c.id; prefill.contactName = c.displayName; }
    else prefill.contactQuery = args.contact;
  }
  if (Array.isArray(args.items) && args.items.length) {
    prefill.items = args.items.map(it => {
      const p = resolveProduct(db, it.product);
      return {
        productId: p ? p.id : '',
        productName: p ? p.name : (it.product || ''),
        weight: it.weight != null ? String(it.weight) : '',
        quantity: it.quantity != null ? String(it.quantity) : '',
        unitPrice: it.unitPrice != null ? String(it.unitPrice) : (p ? String(p.basePrice || '') : ''),
        discount: it.discount != null ? String(it.discount) : '',
      };
    });
  }
  return { ok: true, status: 'ui_builder', builder: { orderType, prefill } };
}
...
const WRITE_TOOLS = [
    ...
    { module: { type: 'string' }, identifier: { type: 'string' } }, ['module', 'identifier']),
  {
    type: 'function',
    function: {
      name: 'open_order_builder',
      description: "Tampilkan FORMULIR INTERAKTIF dengan dropdown untuk membuat Purchase Order (PO) atau Sales Order (SO). Panggil setiap kali user ingin membuat PO/SO. Isi juga argumen prefill (contact, fulfillmentType, items) dari kalimat user JIKA disebutkan, agar formulir sudah terisi awal dan user tinggal menyesuaikan lewat dropdown. Kosongkan/null bila tidak disebut.",
      parameters: {
        type: 'object',
        properties: {
          orderType: { type: ['string', 'null'], description: "'SO' untuk Sales Order, 'PO' untuk Purchase Order, atau null jika belum jelas" },
          contact: { type: ['string', 'null'], description: 'nama/kode customer (SO) atau supplier (PO) bila disebut user' },
          fulfillmentType: { type: ['string', 'null'], description: "khusus SO: 'stock' atau 'dropship' bila disebut" },
          items: {
            type: ['array', 'null'],
            description: 'daftar item yang disebut user (boleh sebagian)',
            items: {
              type: 'object',
              properties: {
                product: { type: 'string', description: 'nama/SKU produk' },
        ...
        required: ['orderType', 'contact', 'fulfillmentType', 'items'],
        additionalProperties: false,
      },
    },
  },
...
function executeTool(name, args, user) {
  ...
  switch (name) {
    ...
    case 'open_order_builder':
      ...
      if (name === 'open_order_builder') return previewOpenOrderBuilder(db, args);
...
8. MEMBUAT ORDER (Purchase Order / Sales Order): PRIORITASKAN memanggil tool open_order_builder untuk menampilkan FORMULIR INTERAKTIF berisi dropdown (pilih customer/supplier/dropshipper/produk/stok, jenis order, dll) sehingga user cukup memilih tanpa mengetik panjang. Tentukan orderType ('SO' atau 'PO') dari maksud user; jika belum jelas, kirim null. ISI argumen prefill (contact, fulfillmentType, items[]) sebanyak yang bisa diambil dari kalimat user (nama customer/supplier, nama produk, berat/jumlah, harga, diskon) agar formulir sudah terisi awal — user tinggal menyesuaikan lewat dropdown. Setelah memanggil open_order_builder, cukup beri kalimat singkat mempersilakan user memeriksa/melengkapi formulir di bawah — JANGAN menanyakan detail satu per satu lewat teks. Gunakan create_purchase_order/create_sales_order HANYA bila user eksplisit minta langsung dibuat tanpa formulir.`;
}
    ...
    while (response.tool_calls && response.tool_calls.length && step++ < MAX_STEPS) {
      for (const call of response.tool_calls) {
        ...
        if (result && result.status === 'ui_builder') {
          uiComponents.push({ id: uuidv4(), type: 'order_builder', orderType: result.builder?.orderType || null, prefill: result.builder?.prefill || null });
