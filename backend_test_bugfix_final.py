#!/usr/bin/env python3
"""
Backend test for SO subtotal bug fix (weight-based calculation) - FINAL VERSION
...
def check_commission_in_db(so_number):
    """Check commission record directly in database"""
    try:
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()
        cursor.execute("SELECT commission_amount, status FROM commission_records WHERE so_number = ?", (so_number,))
...
def test_3_so_with_dropshipper_commission(data):
    ...
    try:
        ...
        if response.status_code == 201:
            ...
            # Get SO detail
            detail_response = session.get(f"{BASE_URL}/sales-orders/{so.get('id')}", timeout=30)
            if detail_response.status_code == 200:
                detail = detail_response.json().get('data', {})
                total = detail.get('totalAmount', 0)
                ...
                print(f"\n📊 RESULTS:")
                print(f"   Total Amount: Rp {total:,.0f}")
                print(f"   Commission Amount (from response): Rp {commission.get('amount', 0):,.0f}")
                
                # Check commission in database
                db_result = check_commission_in_db(so_number)
                if db_result:
                    db_amount, db_status = db_result
                    print(f"   Commission Amount (from DB): Rp {db_amount:,.0f}")
                    print(f"   Commission Status (from DB): {db_status}")
                ...
                if abs(total - expected_total) < 1 and abs(commission.get('amount', 0) - expected_commission) < 1:
                    ...
                    if db_result:
                        print(f"   ✓ Commission saved to database correctly")
                    return True
                else:
    ...
    try:
        ...
        if response.status_code == 201:
            ...
            if all_passed:
                ...
                print(f"\n❌ TEST 5 FAILED: Some commission preview types not working")
            return all_passed
        else:
            print(f"❌ Failed to create SO: {response.text}")
            return False
...
def test_6_commission_payment(data):
    ...
    try:
        # Check DB for unpaid commissions
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()
        cursor.execute("SELECT COUNT(*), SUM(commission_amount) FROM commission_records WHERE dropshipper_id = ? AND status = 'unpaid'", (data['dropshipper_id'],))
        count, total = cursor.fetchone()
        conn.close()
        
        print(f"Unpaid commissions in DB: {count} records, Total: Rp {total or 0:,.0f}")
        ...
        if count > 0:
            ...
            if payment_response.status_code == 200:
                ...
                print(f"\n📊 RESULTS:")
                print(f"   Unpaid commissions after payment: {count_after} records, Total: Rp {total_after or 0:,.0f}")
                
                if count_after == 0:
                    print(f"✅ TEST 6 PASSED: All commissions paid, outstanding = 0")
                    return True
                else:
                    print(f"❌ TEST 6 FAILED: Still have {count_after} unpaid commissions")
                    return False
            else:
                print(f"❌ Payment failed: {payment_response.text}")
                return False
        else:
            print(f"⚠️  No unpaid commissions to pay")
...
def main():
    ...
    # Test 1: SO total (weight-based, no discount)
    results["test_1"] = test_1_so_weight_based_no_discount(data)
    
    # Test 2: SO with Agen discount
    results["test_2"] = test_2_so_with_agen_discount(data)
    
    # Test 3: SO with dropshipper commission
    results["test_3"] = test_3_so_with_dropshipper_commission(data)
    
    # Test 4: SO PATCH items recompute
    results["test_4"] = test_4_so_patch_items_recompute(data)
    ...
    if passed == total:
        ...
        print("\n✅ KEY FINDINGS:")
        print("   • Weight-based calculation working correctly (unitPrice * weight)")
        print("   • SO totals are POSITIVE with Agen discount (bug fixed)")
        print("   • Commission auto-creation working correctly")
        print("   • Commission calculations accurate for all types")
        print("   • PATCH items recompute working correctly")
    else:
